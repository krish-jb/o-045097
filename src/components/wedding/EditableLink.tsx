...
const EditableLink: React.FC<EditableLinkProps> = ({
    ...
    if (!isLoggedIn) {
        return (
            <button
                onClick={goToMaps}
                className={`-mx-1 px-1 text-orange-500 shadow-[inset_0_0_0_0_#ff6900] transition-all duration-300 ease-in-out hover:text-white italic hover:shadow-[inset_50rem_0_0_0_#ff6900] max-w-xs md:max-w-full truncate ${className}`}
                type="button"
            >
                {children || text}
            </button>
